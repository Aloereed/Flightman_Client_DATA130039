...
void account_and_orders::checkNewMsg(){
    QSqlQuery ck(QString("SELECT time,text FROM announcement WHERE userID = '%1'").arg(this->UserID));
...
void account_and_orders::on_MessageBox_pushButton_clicked()
{
    ...
    QSqlQuery ck(QString("SELECT time,text FROM announcement WHERE userID = '%1'").arg(acct->getUserID()));
    ck.next();
